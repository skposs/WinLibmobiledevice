...
#ifdef WIN32
#include <winsock2.h>	// windows.h 헤더와 순서 변경
#include <windows.h>
				...
				*payload = (void*)dev;
				...
				if (n) {
					...
					*payload = malloc(sizeof(uint32_t));
					...
					memcpy(*payload, &dwval, sizeof(dwval));
					hdr.length = sizeof(hdr) + sizeof(dwval);
					hdr.message = MESSAGE_DEVICE_PAIR;
				}
			}else {
...
static int get_next_event(int sfd, usbmuxd_event_cb_t callback, void *user_data)
{
	...
	else if (hdr.message == MESSAGE_DEVICE_PAIR) {
		struct usbmuxd_connect_request *dev = (usbmuxd_connect_request *)payload;
		generate_event(callback, NULL, UE_DEVICE_PAIR, user_data);
