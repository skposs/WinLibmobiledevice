...
static plist_t parse_dict_node(struct bplist_data *bplist, const char** bnode, uint64_t size)
{
    ...
    for (j = 0; j < data->length; j++) {
        ...
        index1 = UINT_TO_HOST((char *)(*bnode) + str_i, bplist->dict_size);
        index2 = UINT_TO_HOST((char *)(*bnode) + str_j, bplist->dict_size);
...
static plist_t parse_array_node(struct bplist_data *bplist, const char** bnode, uint64_t size)
{
    ...
    for (j = 0; j < data->length; j++) {
        ...
        index1 = UINT_TO_HOST((char *)(*bnode) + str_j, bplist->dict_size);
...
static plist_t parse_bin_node_at_index(struct bplist_data *bplist, uint32_t node_index)
{
    ...
    ptr = bplist->data + UINT_TO_HOST((char *)bplist->offset_table + node_index * bplist->offset_size, bplist->offset_size);
